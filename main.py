...
from newOSC import TouchLiveOscBackend
...
class TouchLiveMain(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.newOSC = TouchLiveOscBackend()
        ...
        self.newOSC.setOSCHandler('/%s/led' % prefix, self.on_led)
        self.newOSC.setOSCHandler('/%s/led_col' % prefix, self.on_led_col)
        self.newOSC.setOSCHandler('/%s/led_row' % prefix, self.on_led_row)
        self.newOSC.setOSCHandler('/%s/led_frame' % prefix, self.on_led_frame)
        self.newOSC.setOSCHandler('/%s/led_clear' % prefix, self.on_led_clear)

        # empty handlers
        self.newOSC.setOSCHandler('/sys/prefix', self.empty_handler)
        self.newOSC.setOSCHandler('/sys/cable', self.empty_handler)
        self.newOSC.setOSCHandler('/sys/offset', self.empty_handler)
        self.newOSC.setOSCHandler('/sys/intensity', self.empty_handler)
        self.newOSC.setOSCHandler('/sys/test', self.empty_handler)
        self.newOSC.setOSCHandler('/sys/report', self.empty_handler)

    def on_button_state(self, instance, value):
        prefix = self.app.config.get('monome', 'prefix')
        index = instance.index
        value = 1 if value == 'down' else 0
        self.newOSC.sendOSCMsg('/%s/press' % prefix, [index % 8, index / 8, value])
    ...
    def on_led_col(self, addr, tags, data, source):
        col, cv = data[0:2]
        colvals = map(int, (list(''.join([str((cv>> y) & 1) for y in range(7, -1, -1)]))))
        colvals.reverse()
        for i in range(8):
            index = int(col + (8 * i))
...
class TouchLiveApp(App):
    ...
    def do_connect(self):
        ...
        # osc
        self.root.newOSC.initOSCClient(host, sport)
        self.root.newOSC.initOSCServer(host, rport)
